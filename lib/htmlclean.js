...
module.exports = function(html, options) {
  ...
  // <!--[htmlclean-protect]-->
  html = html.replace(/<[^\S\f]*\!--[^\S\f]*\[[^\S\f]*htmlclean-protect[^\S\f]*\][^\S\f]*--[^\S\f]*>([\s\S]*?)<[^\S\f]*\!--[^\S\f]*\[[^\S\f]*\/[^\S\f]*htmlclean-protect[^\S\f]*\][^\S\f]*--[^\S\f]*>/ig,
    function(str, p1) { return protectedTextAdd(p1); });

  // Apache SSI tags.
  html = html.replace(/(<[^\S\f]*\!--[^\S\f]*\#[\s\S]*?--[^\S\f]*>)/g, // Not [^>]. '>' may be included.
    function(str, p1) { return protectedTextAdd(p1); });

  // IE conditional comments.
  // The line-breaks will not be used for margins of these elements.
  // html = html.replace(/(?:[\t ]*[\n\r][^\S\f]*)?(<[^\S\f]*\!(?:--)?[^\S\f]*\[[^\S\f]*if\b[^>]*>)(?:[\t ]*[\n\r][^\S\f]*)?/ig,
  //   function(str, p1) { return protectedTextAdd(p1); });
  // html = html.replace(/(?:[\t ]*[\n\r][^\S\f]*)?(<[^\S\f]*\![^\S\f]*\[[^\S\f]*endif\b[^>]*>)(?:[\t ]*[\n\r][^\S\f]*)?/ig,
  //   function(str, p1) { return protectedTextAdd(p1); });
  ...
  html = html.replace(/<([^>]+)>/g,
    function(str, tagInner) {
      if (/^[^\S\f]*\![^\S\f]*--[\s\S]*--[^\S\f]*$/.test(tagInner)) {
        return '';
      } else {
